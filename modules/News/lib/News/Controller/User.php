...
class News_Controller_User extends Zikula_AbstractController
{
    ...
    public function view($args = array())
    {
        ...
        // The items that are displayed on view pages depend on the user permissions: $accesslevel
        $this->view->setCacheId('view|prop_'.$prop.'_cat_'.$catparam . '|page_'.$page.'_'.$itemsperpage.'_'.$accesslevel);

        // if page is cached return cached content, insted to make more queries to database
        $template1 = 'user/'.$giventemplate;
        if ($this->view->is_cached($template1)) {
            return $this->view->fetch($template1);
        }
        ...
        foreach ($items as $item) {
            ...
            if (($item['published_status'] == 0) &&
                ...
                $newsitems[] = $this->view->fetch($template, $item['sid'].'|a'.$accesslevel);
            }
        }
    ...
    public function display($args)
    {
        ...
        if ((!isset($sid) || empty($sid)) && !empty($title)) {
            // get sid by given title
            $item = DBUtil::selectObjectByID('news', $title, 'urltitle', array('sid'));
            $sid = $item['sid'];
        ...
        if ($numpages <= 1) {
            ...
            $info['pagetitle'] = $info['title'] . ' :: ' . $this->__f('page %s', $page);
        }
    ...
    public function categorylist($args)
    {
        ...
        // The items that are displayed on this pages depend on the user articles
        $this->view->setCacheId('categorylist|uid_'.$uid);

        // if page is cached return cached content, insted to make more queries to database
        $template = 'user/categorylist.tpl';
        if ($this->view->is_cached($template)) {
            return $this->view->fetch($template);
        }
